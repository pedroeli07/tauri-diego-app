...
const DebugBox: React.FC<DebugBoxProps> = ({
  ...
  // Função para evitar logs duplicados consecutivos
  ...
  // Sincronizar com logs do backend
  useEffect(() => {
    const handleBackendLog = (event: any) => {
      const { message, type } = event.detail;
      addUniqueLog(message, type);
    };

    window.addEventListener("backendLog", handleBackendLog);

    return () => {
      window.removeEventListener("backendLog", handleBackendLog);
    };
  }, []);
